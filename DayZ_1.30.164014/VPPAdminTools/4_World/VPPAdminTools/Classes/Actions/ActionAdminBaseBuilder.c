	...
	protected bool BuildCheckSimpleClient(PlayerBase player, ActionTarget target, ItemBase item)
	{
		...
		if (base_building)
		{
			...
			if (construction && BuildCondition(player, target))
			{
				...
				CollisionCheckData checkData = new CollisionCheckData();
				checkData.m_PartName = partName;
				checkData.m_AdditionalExcludes.Insert(player);
				return !construction.IsCollidingEx(checkData);
			}
		}
		return false;
	}
